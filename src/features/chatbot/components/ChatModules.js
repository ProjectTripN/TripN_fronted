...
export function WithWho(props) {
  ...
  useEffect(() => {
    props.steps.travel_with.message != "해당 없음" ? (
      ...
      dispatch(
        withWho({ sentence: props.steps.userinput_travel_with_other.value })
      )
    );
  }, [text]);
  // window.localStorage.setItem("relationship", text)
      ...
      {props.steps.travel_with.message == "혼자" ? (
        ...
        <div>
          {text != "" ? (
            ...
            <>
              {text_2}
              {window.localStorage.setItem("relationship", text_2)}
            </>
          )}
          와 함께 가시는 군요.
        </div>
  ...
  return (
    <div>
      ...
      <input
        id="start"
        type="date"
        min={`${a.getFullYear()}-${("0" + (a.getMonth() + 1)).slice(-2)}-${(
          "0" + a.getDate()
        ).slice(-2)}`}
      />
      <br />
...
export function CheckDate(props) {
  return (
    <div
      onChange={() => {
        props.triggerNextStep({ trigger: "check_check" });
      }}
    >
      이번 여행은
      {localStorage.getItem("relationship") == "혼자" ? (
        ...
        <>
          {localStorage.getItem("number") == 1 ? (
            <p>
              {localStorage.getItem("relationship")}와(과) 함께 하는 여행으로,
            </p>
          ) : (
            <p>
              {localStorage.getItem("relationship")}와(과) 함께{" "}
              {localStorage.getItem("number")}명에서 떠나는 여행으로,{" "}
            </p>
          )}
        </>
      )
      ...
      {localStorage.getItem("start")}에 {localStorage.getItem("airstartword")}
      에서 출발하여, {localStorage.getItem("end")}까지의 일정 맞나요?
    </div>
  );
}
...
export function Ask(props) {
  ...
  const botAnswer = {
    [key]: useSelector((state) => state.chatSlice.answersState[key]),
  };
  useEffect(() => {
    setText(props.steps.question.value);
    dispatch(
      answer({ key: props.previousStep.key, value: props.steps.question.value })
    );
  }, [text]);
  return (
    <>
      {text == "여행 예약하기" ? (
        <div>
          여행을 예약하시려면 '여행 예약 시작' 버튼을, <br /> 상담을 하시려면
          '취소' 버튼을 눌러주세요.
          <button
            onClick={() =>
              props.triggerNextStep({ trigger: "welcome_reservation" })
            }
            style={{ background: "#bce7d6", margin: "3px" }}
          >
            여행 예약 시작
          </button>
        ...
        여행 예약</button> */}
          <button
            onClick={() => props.triggerNextStep({ trigger: "welcome_2" })}
            style={{ background: "#bce7d6", margin: "3px" }}
          >
            취소
          </button>
